...
void init()
{
    ...
    initTexture();
    initToilets(wallTexture);

    // Load texture beton untuk building
    int w2, h2, ch2;
    unsigned char* d2 = stbi_load("assets/beton_texture.jpg", &w2, &h2, &ch2, 0);
    glGenTextures(1, &bldgTexture);
    glBindTexture(GL_TEXTURE_2D, bldgTexture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    if (d2) {
        GLenum fmt = (ch2 == 4) ? GL_RGBA : GL_RGB;
        glTexImage2D(GL_TEXTURE_2D, 0, fmt, w2, h2, 0, fmt, GL_UNSIGNED_BYTE, d2);
        gluBuild2DMipmaps(GL_TEXTURE_2D, fmt, w2, h2, fmt, GL_UNSIGNED_BYTE, d2);
        stbi_image_free(d2);
    }
